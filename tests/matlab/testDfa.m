...
x = quarctest.signals('white', 4096);
a = localAlpha(x);
...
x = quarctest.signals('brown', 4096);
a = localAlpha(x);
...
got = zeros(size(Hs));
for k = 1:numel(Hs)
    got(k) = localAlpha(quarctest.signals('fgn', 4096, Hs(k)));
end
...
x = quarctest.signals('fgn', 2048, 0.75);
a1 = localAlpha(x);
...
x = quarctest.signals('white', 1024);
sc = localScales(1024);
s = quarctest.sideEffects(@() dfa(x, sc, 1, false));
...
x = quarctest.signals('white', 1024);
sc = localScales(1024);
s = quarctest.sideEffects(@() dfa(x, sc, 1));
