...
x = quarctest.signals('ar1', 512, 0.7);
...
white = ent_samp(quarctest.signals('white', 2000), 2, 0.2);
ar1   = ent_samp(quarctest.signals('ar1', 2000, 0.9), 2, 0.2);
...
got = arrayfun(@(p) ent_samp(quarctest.signals('ar1', 2000, p), 2, 0.2), phis);
...
sine = ent_samp(quarctest.signals('sine', 2000, 25), 2, 0.2);
white = ent_samp(quarctest.signals('white', 2000), 2, 0.2);
...
x = quarctest.signals('ar1', 1000, 0.5);
...
x = quarctest.signals('ar1', 500, 0.5);
...
s = quarctest.sideEffects(@() ent_samp(quarctest.signals('white', 500), 2, 0.2));
